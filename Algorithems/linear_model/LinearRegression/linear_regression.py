...
def linear_regression(x, y, epochs=1000, lr_rate=1e-2): # Here x is the feature and y is the target while Epochs refer is each time we go through the data set and tune the weight and bias
    ...
    w = np.random.rand() # We start with a random weight and initially adjust it with gradient descent (Optimization algorithem). 
                         # This determines how strong a input feature affects the target
   
    b = 0.0 # Similarly we start with the origin. The bias basically contorls the position of the line.
